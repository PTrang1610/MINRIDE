...
public abstract class BaseManager<T extends IEntity> implements IRepository<T>{
    ...
    protected Stack<Operation> undoStack = new Stack<>();

    // Kiểm tra entity và ID
    protected void validateEntity(T entity) {
        if (entity == null || entity.getId() == null || entity.getId().isEmpty()) {
            throw new IllegalArgumentException("Entity or ID cant be null or empty");
        }
    }   
    ...
    public void Add(T entity) {
        ...
        if (isDuplicateId(entity.getId())) {
            throw new IllegalArgumentException("ID " + entity.getId() + " is already in use");
        }
        entities.add(entity);
    ...
    public void Update(String id, T updatedEntity) {
        ...
        if (!id.equals(updatedEntity.getId()) && isDuplicateId(updatedEntity.getId())) {
            throw new IllegalArgumentException("ID " + updatedEntity.getId() + " is already in use");
        }
    ...
    public List<T> findByName(String name) {
        if (name == null || name.isEmpty()) {
            throw new IllegalArgumentException("Tên cant be null or empty");
        }
    ...
    protected void saveOperation(String type, T entity) {
        undoStack.push(new Operation(type, entity));
    }
    ...
    protected void undo() {
        ...
        System.out.println("Undo successfully: " + op.type);
    }
