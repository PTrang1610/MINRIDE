...
public class RideManager extends BaseManager<Ride> {
    ...
    //Polymorphism
    public void addRide(Ride entity) {  //T -> Ride
        super.Add(entity);
    ...
    public Ride autoAssignDriver(String customerId, CustomerManager customerManager,
                                 DriverManager driverManager, double radius) {
        if (driverManager == null) {
            return null;
        }

        List<Driver> nearbyDrivers = driverManager.findNearByDrivers(customerManager, customerId,  radius);

        if (nearbyDrivers.isEmpty()) {
            return null;
        }

        Driver bestDriver = nearbyDrivers.getFirst();
    ...
    public Map<String, Object> getRideStatistics() {
        ...
        // Base Statistical
        stats.put("Tổng số chuyến:", allRides.size());
        stats.put("Tổng doanh thu!", calculateTotalRevenue());
        stats.put("Số chuyến đang chờ!", pendingRides.size());
